...
namespace Timberborn_FloodSeason
{
	class HazardousWeatherRandomizerReplacement
	{
		...
		public HazardousWeatherRandomizerReplacement(DroughtWeather droughtWeather, BadtideWeather badtideWeather, FloodWeather floodWeather, IRandomNumberGenerator randomNumberGenerator)
		{
		...
		public IHazardousWeather GetRandomWeatherForCycle(int cycle)
		{
			...
			if (roll < 0.25 && _floodWeather.CanOccurAtCycle(cycle)) // fixed 25% chance
			{
			...
			else
			{
				...
				result = _badtideWeather.CanOccurAtCycle(cycle) && roll < _badtideWeather.ChanceForBadtide ? _badtideWeather : result;
			}
